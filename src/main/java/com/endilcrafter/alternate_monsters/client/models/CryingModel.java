package com.endilcrafter.alternate_monsters.client.models;
...
@OnlyIn(Dist.CLIENT)
public class CryingModel<T extends Entity> extends HierarchicalModel<T> {
    private final ModelPart root;
    private final ModelPart[] upperBodyParts;
    private final ModelPart head;

    public CryingModel(ModelPart pRoot) {
        this.root = pRoot;
    ...
    public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        f = (float) (Math.PI / 4) + pAgeInTicks * (float) Math.PI * 0.03F;
